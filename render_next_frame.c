...
/*   Updated: 2022/01/31 18:04:33 by pleveque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_point(t_screen *screen_data, float x, float y, float z, t_mat4x4 *mat_rot_x, t_mat4x4 *mat_rot_z)
{
	...
	coord = rotate_z(mat_rot_z, coord);
	coord = rotate_x(mat_rot_x, coord);
	coord->z += 3;
...
int	render_next_frame(t_screen *screen_data)
{
	...
	mat_rot_x = screen_data->mat_rot_x;
	mat_rot_z = screen_data->mat_rot_z;
	...
	// 	y = 0;
		...
		// 	while (x <= size)
		// 	{
		// 		draw_point(screen_data, 2 / size * x - 1, 2 / size * y - 1, 0, mat_rot_x, mat_rot_z);
		// 		++x;
		// 	}
		// 	++y;
		// }
	t_list *vectors = (t_list* )screen_data->vectors;
	while (vectors->next)
	{
		vectors = vectors->next;
		draw_point(screen_data, ((t_vec3d *)vectors->content)->x, 
			((t_vec3d *)vectors->content)->y / size - 1, 
			((t_vec3d *)vectors->content)->z / size / 2, 
			mat_rot_x, mat_rot_z);
		//printf("%f, %f, %f\n",((t_vec3d *)vectors->content)->x, ((t_vec3d *)vectors->content)->y, ((t_vec3d *)vectors->content)->z);
	}
	// 	++z;
	coords[0] = 0;
	coords[1] = 0;
	coords[2] = 0;
	coords[3] = 0;
	coords[0] = vec3d_coord(coords[0], -1, -1, -1);
	coords[1] = vec3d_coord(coords[1], 1, -1, -1);
	coords[2] = vec3d_coord(coords[2], 1, 1, -1);
	coords[3] = vec3d_coord(coords[3], -1, 1, -1);
	//}
...
// //back ================================================
	coords[0] = vec3d_coord_upd(coords[0], -1, -1, 1);
	coords[1] = vec3d_coord_upd(coords[1], 1, -1, 1);
	coords[2] = vec3d_coord_upd(coords[2], 1, 1, 1);
	coords[3] = vec3d_coord_upd(coords[3], -1, 1, 1);
